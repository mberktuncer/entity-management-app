...
@Qualifier(QUALIFIER_GOLD)
public class GoldTransactionServiceImpl implements TransactionService {
    ...
    public List<RetrieveTransactionResponse> findAll() {
        return transactionRepository.findAll()
                .stream()
                .filter(transaction -> transaction.getAccountType().equals(ACCOUNT_TYPE_GOLD))
    ...
    public List<RetrieveTransactionResponse> findAllByAccountId(String accountId) {
        var entity = accountServiceHelper.findEntityById(accountId, ACCOUNT_TYPE_GOLD);
        return transactionRepository.findAll()
                .stream()
    ...
    public UpdateTransactionResponse addMoney(UpdateTransactionRequest updateTransactionRequest) {
        var goldAccount = (GoldAccount) accountServiceHelper.findEntityById(updateTransactionRequest.getReceiverAccountNumber(),ACCOUNT_TYPE_GOLD);
        var existingBalance = goldAccount.getBalance();
        var newBalance = existingBalance.add(updateTransactionRequest.getTransferAmount());
        transactionServiceHelper.saveFinalAmountToAccountByAccountType(goldAccount, ACCOUNT_TYPE_GOLD, newBalance);
        return transactionServiceHelper.mapToUpdateTransactionResponse(updateTransactionRequest, goldAccount, newBalance);
    }
